...
def create_grid( x, y, void_spaces):
	board = [[Cell(type = CellType.Empty, pos=[ix,iy]) for iy in range(y)] for ix in range(x)]
	print("void_spaces = %s" % void_spaces)
	for x,y in void_spaces:
	    board[x][y].type = CellType.Block

	board[0][0].type = CellMark.Start

	return types.SimpleNamespace( 
		board = CellGrid(board),
		start = [0,0] ), void_spaces
