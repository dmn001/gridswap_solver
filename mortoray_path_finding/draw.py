...
pygame.init()
pygame.display.set_caption("Gridswap Solver")
...
def main_loop(ui):
    ...
    ui.set_display(pygame.display)
    ui.set_screen(screen_x,screen_y)
...
def draw_board(surface, area, board):
    ...
    colors = {
        maze.CellType.Empty: (103,201,77),
        maze.CellType.Block: (0,0,0),
    }
    marks = {
        maze.CellMark.Start: (255,255,255),
        maze.CellMark.End: (0,110,0),
    }
    for y in range(0,metrics.num_y):
        for x in range(0,metrics.num_x):
            cell = board.at([x,y])
            clr = colors.get(cell.type, (255,255,255))
            cell_rect = metrics.cell_rect( [x, y] )

            pygame.draw.rect(surface, clr, cell_rect)

            # # uncomment to render coordinates:
            # number = cell_font.render( "(%s,%s)" % (x,y), True, (255,255,255))
            # surface.blit(number, trans_rect(number.get_rect(), 
            #   [cell_rect[0] + (cell_rect[2] - number.get_rect()[2])/3, 
            #   cell_rect[1] + (cell_rect[3] -number.get_rect()[3])/3]
            # ))
...
def draw_path(surface, area, board, path):
    ...
    for i in range(0,len(path)-1):
        ...
        pygame.draw.line(surface, (255,0,0),  ctr_a, ctr_b, metrics.spacing )
